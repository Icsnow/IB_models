...
class IB_ForkSKINNY:
  ...
  def __init__(self, b_size, Vs, ri, r0, r1, rEb, rEu, rEl, rEf, Vx, cP) -> None:
    '''
    ...
    ***************+***********************************************************
    ...
    self.name = './ForkSKINNY-{}-{}_{}r'.format(self.b_size, self.k_size, self.rEb + self.rEu + self.rEl + self.rEf)
  ...
  def ib_model(self):
    ...
    udw  = self.model.addVars(uR,             16, vtype = GRB.BINARY, name = 'udw') # w0 = Plaintext
    ustk = self.model.addVars(uR+self.r0,     16, vtype = GRB.BINARY, name = 'ustk')
    ...
    for c in [[0,4,8,12],[1,5,9,13],[2,6,10,14],[3,7,11,15]]:
        ...
        self.model.addConstr(eqk[c0] >= ustk[0,tc3], name = 'eqk0')
        self.model.addConstr(eqk[c0] >= eqk[c3], name = 'eqk0')
        self.model.addConstr(eqk[c0] <= ustk[0,tc3] + eqk[c3], name = 'eqk0')
        ...
        self.model.addConstr(eqk[c2] >= ustk[0,tc2], name = 'eqk2')
        self.model.addConstr(eqk[c2] <= ustk[0,tc1] + ustk[0,tc2], name = 'eqk2')

        # --- POSITION -- 2: eqk[c3] = stk1[c0] + stk[c2] ---
        self.model.addConstr(eqk[c3] >= ustk[0,tc0], name = 'eqk2')
        self.model.addConstr(eqk[c3] >= ustk[0,tc2], name = 'eqk2')
        self.model.addConstr(eqk[c3] <= ustk[0,tc0] + ustk[0,tc2], name = 'eqk2')
    ...
    if (uR + self.r0) <= 30:
      for i in range(16):
        ...
        self.model.addConstr((uR+self.r0)*uLANE[i] - total_stack_sum <= (self.Vs - 1), name = 'uKS')
    else:
      for i in range(16):
        lane_pos = i
        total_stack_sum_ri = 0
        total_stack_sum_r1 = 0
        for r in range(30):
          self.model.addConstr(uLANE[i] >= ustk[r, lane_pos], name = 'uKS')
          total_stack_sum_ri += ustk[r, lane_pos]
          lane_pos = self.hTable[lane_pos]
        for r in range(30, uR+self.r0):
          self.model.addConstr(uLANE[i] >= ustk[r, lane_pos], name = 'uKS')
          total_stack_sum_r1 += ustk[r, lane_pos]
          lane_pos = self.hTable[lane_pos]

        self.model.addConstr(30*uLANE[i] - total_stack_sum_ri <= (self.Vs - 1))
        self.model.addConstr((uR+self.r0-30)*uLANE[i] - total_stack_sum_r1 <= (self.Vs - 1))
    # make Key reuse
    for r in range(30, uR + self.r0):
        for i in range(16):
    ...
    X -(SC&AC)-> Y -(ATK)-> Z -(SR)-> W -(MC)-> X^{r+1}
    '''
    lR = self.rEl + self.rEf
    lDX  = self.model.addVars(range(uR+1,uR+lR+1),              16, vtype = GRB.BINARY, name = 'lDX')
    ldx  = self.model.addVars(range(uR+1,uR+lR+1),              16, vtype = GRB.BINARY, name = 'ldx')
    lDY  = self.model.addVars(range(uR,uR+lR),                  16, vtype = GRB.BINARY, name = 'lDY')
    ldy  = self.model.addVars(range(uR,uR+lR),                  16, vtype = GRB.BINARY, name = 'ldy')
    lDZ  = self.model.addVars(range(uR,uR+lR),                  16, vtype = GRB.BINARY, name = 'lDZ') # DZ = Ciphertext
    ldz  = self.model.addVars(range(uR,uR+lR),                  16, vtype = GRB.BINARY, name = 'ldz') # dz = Ciphertext
    lDW  = self.model.addVars(range(uR,uR+lR),                  16, vtype = GRB.BINARY, name = 'lDW')
    ldw  = self.model.addVars(range(uR,uR+lR),                  16, vtype = GRB.BINARY, name = 'ldw')
    lstk = self.model.addVars(range(uR+self.r0, uR+self.r0+lR), 16, vtype = GRB.BINARY, name = 'lstk')

    '''Basic Constrs:'''
    # Remove dx[r,i] > DX[r,i]
    for r in range(uR+1,uR+lR):
      for i in range(16):
    ...
    for i in range(16):
      self.model.addConstr(lDY[uR, i] >= ldy[uR, i], name='lBasic')
      self.model.addConstr(lDZ[uR, i] >= ldz[uR, i], name='lBasic')
      self.model.addConstr(lDW[uR, i] >= ldw[uR, i], name='lBasic')
    for i in range(16):
      self.model.addConstr(lDX[uR+lR, i] >= ldx[uR+lR, i], name='lBasic')

    '''Operation: SC'''
    # In distinglisher
    for r in range(uR+1, uR+self.rEl):
      for i in range(16):
        self.model.addConstr(lDX[r,i] == lDY[r,i], name='ldSC')
        self.model.addConstr(ldx[r,i] == lDY[r,i], name='ldSC')
    # In Extension
    for r in range(uR+self.rEl, uR+lR):
      for i in range(16):
        self.model.addConstr(lDY[r,i] == lDX[r,i], name='leSC')
        self.model.addConstr(ldy[r,i] == lDX[r,i], name='leSC')
    
    '''(*)Operation: ATK'''
    for r in range(uR,uR+lR):
      for i in range(8):
        tlr = self.r0 + r
        self.model.addConstr(ldy[r,i] == ldz[r,i], name='lATK')
        self.model.addConstr(lstk[tlr,i] + lDY[r,i] - lDZ[r,i] >= 0, name='lATK')
        self.model.addConstr(lstk[tlr,i] - lDY[r,i] + lDZ[r,i] >= 0, name='lATK')
        self.model.addConstr(-lstk[tlr,i] + lDY[r,i] + lDZ[r,i] - ldz[r,i] >= 0, name='lATK')
        self.model.addConstr(lDY[r,i] >= ldz[r,i], name='lATK')
      for i in range(8,16):
        self.model.addConstr(lDY[r,i] == lDZ[r,i], name='lATK')
        self.model.addConstr(ldy[r,i] == ldz[r,i], name='lATK')
    ...
    for i in range(16):
      lane_pos = i
      kSUM = 0
      for r in range(uR+self.r0, uR+self.r0+lR):
    ...
    totR = uR+lR
    # NOTE: Assert rEb <= ri
    lGstk = self.model.addVars(range(leR+self.r0, totR+self.r0), 16, vtype = GRB.BINARY, name = 'lGstk')
    for r in range(leR+self.r0, totR+self.r0): 
      for i in range(8,16):
    ...
    # ----------------------------------------------------------------------------------------------------
    lgstka = self.model.addVars(range(leR+self.r0, totR+self.r0), 16, vtype = GRB.BINARY, name = 'lgstka')
    for r in range(leR+self.r0, totR+self.r0): 
      for i in range(8,16):
    ...
    for r in range(leR, totR):
      rk = r + self.r0
      for i in range(8):
        self.model.addConstr(lDetY[r,i] <= lGstk[rk,i], name = 'lgkiU')
        self.model.addConstr(lDetY[r,i] <= lDetZ[r,i], name = 'lgkiU')
        self.model.addConstr( - lGstk[rk,i] - lDetZ[r,i] + lDetY[r,i] + 1 >= 0, name = 'lgkiU')
        # -------------------------------------------------------------------------------------
        self.model.addConstr(ldetYa[r,i] <= lgstka[rk,i], name = 'lgkiUa')
        self.model.addConstr(ldetYa[r,i] <= ldetZa[r,i], name = 'lgkiUa')
        self.model.addConstr( - lgstka[rk,i] - ldetZa[r,i] + ldetYa[r,i] + 1 >= 0, name = 'lgkiUa')
      for i in range(8,16):
    ...
    for i in range(16):
      self.model.addConstr(iLANE[i] == sum(ugstka[r,self.iterate_hTable(i,r)] for r in range(self.rEb)) + 
                                       sum(lgstka[r,self.iterate_hTable(i,r)] for r in range(leR+self.r0, totR+self.r0)), 
                                       name = 'KeyBgeI')
    for i in range(16):
    ...
    for i in range(16):
      self.model.addConstr(gLANE[i] == sum(uGstk[r,self.iterate_hTable(i,r)] for r in range(self.rEb)) + 
                                       sum(lGstk[r,self.iterate_hTable(i,r)] for r in range(leR+self.r0, totR+self.r0)), 
                                       name = 'KeyBgeG')
    for i in range(16):
    ...
    # ******************************************************
    ''' Contradiction '''
    self.model.addConstr(sum((uDX[uR,i] - udx[uR,i]) * (lDY[uR,i] - ldy[uR,i]) for i in range(16)) >= 1, name = 'iBCT')
    # ******************************************************
...
if __name__ == '__main__':
  ...
  SOl = [(6, 27, 18, 4, 8, 7, 5), (7, 27, 17, 4, 9, 6, 5), (8, 27, 16, 4, 10, 5, 5), (7, 27, 17, 5, 8, 7, 4),
         (8, 27, 16, 5, 9, 6, 4), (9, 27, 15, 5, 10, 5, 4), (6, 27, 18, 3, 9, 6, 6), (7, 27, 17, 3, 10, 5, 6), 
         (8, 27, 16, 6, 8, 7, 3), (9, 27, 15, 6, 9, 6, 3)]
  test_case = 0
  ri, r0, r1, rEb, rEu, rEl, rEf = SOl[test_case]
  FS128v2 = IB_ForkSKINNY(128,2, ri,r0,r1, rEb,rEu,rEl,rEf, 48, True)
  FS128v2.ib_model()