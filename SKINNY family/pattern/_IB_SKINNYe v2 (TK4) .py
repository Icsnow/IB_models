...
class IB_ForkSKINNY:
  ...
  def __init__(self, b_size, Vs, ri, r0, r1, rEb, rEu, rEl, rEf, Vx, cP) -> None:
    ...
    ***************+***********************************************************
    ...
    self.name = './SKINNYe-{}-{}_{}-{}+{}+{}+{}_{}'.format(
      self.b_size, self.k_size, rEb+rEl+rEu+rEf, rEb, rEu, rEl, rEf, cP
    )
  ...
  def ib_model(self):
    ...
    for i in range(16):
      ...
      self.model.addConstr((uR+self.r0)*uLANE[i] - total_stack_sum <= 
                            (self.Vs - 1)*math.ceil((uR + self.r0)/30), name = 'uKS')
    ...
    self.model.addConstr(mfi == sum(lgstka[r,i] for r in range(leR+self.r0, totR+self.r0) for i in range(8)), name = 'mf')
...
if __name__ == '__main__':
  ...
  SKINNYe = IB_ForkSKINNY(64, 4,  ri, r0, r1,  rEb, rEu, rEl, rEf,  32, False)
  SKINNYe.ib_model()
