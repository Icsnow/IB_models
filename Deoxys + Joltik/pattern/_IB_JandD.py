...
class IB_DandJ:
  ...
  def __init__(self, key_size, round_Eb, round_Eu, round_Em, round_El, round_Ef, setX, chooseP) -> None:
  ...
  def ib_model(self):
    ...
    *PARA.: 
    ...
    '''
    Add Counstraints for eps
    '''
    # for c in range(4):
      # self.model.addConstr(sum(udx[1,4*c+i] for i in range(4)) - sum(udy[1,4*c+i] for i in range(4)) >= 2)
    # self.model.addConstr(mfp <= mf - 1)
    # self.model.addConstr(sum(udw[0,4*c+i] for i in range(4) for c in range(4)) <= 11)
    # self.model.addConstr(T31 - T32 <= 24)
    '''
    ===================================================
    Solve model
    ===================================================
    ...
    print('>>>>> Solution <<<<<')
    print('='*90)
    print('|| Choosen Plaintext | ' if self.choosep else('Choosen Chiphertext | '), end='')
...
if __name__ == '__main__':
    ...
    # IB_Deoxys(key_size, round_Eb, round_Eu, round_Em, round_El, round_Ef, setX, chooseP)
